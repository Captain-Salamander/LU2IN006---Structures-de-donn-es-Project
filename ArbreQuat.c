...
void insererNoeudArbre(Noeud* n, ArbreQuat** a, ArbreQuat* parent){
    ...
    if((*a)==NULL){
        ...
        double xc, yc;
        //Si y du noeud est superieur au centre de l'arbre parent => L'arbre se situe au nord
        if((n->y) >= (parent->yc)){
            yc = (parent->yc) + (parent->coteY)/4;
        }else{ //Sinon, il se situe au sud
            yc = (parent->yc) - (parent->coteY)/4;
        }
            
        //Si x du noeud superieur au centre du parent => L'arbre se situe a l'est
        if((n->x) >= (parent->xc)){
            xc = (parent->xc) + (parent->coteX)/4;
        }else{ //Sinon, il se situe a l'ouest
            xc = (parent->xc) - (parent->coteX)/4;
        }
    ...
    if((*a)->noeud != NULL){
        ...
        ArbreQuat **afilsd = direction(deplace->x, deplace->y, *a); //Direction dans laquelle placer le noeud deplacé
        insererNoeudArbre(deplace, afilsd, *a); //Insertion du noeud deplacé de la feuille depuis le parent

        ArbreQuat **afilsn = direction(n->x, n->y, *a); //Direction dans laquelle placer le nouveau noeud
        insererNoeudArbre(n, afilsn, *a);
        return;
    }
...
Noeud* rechercheCreeNoeudArbre(Reseau* R, ArbreQuat** a, ArbreQuat* parent, double x, double y){
    ...
    //Cas où a est une cellule interne
    if((*a != NULL) && ((*a)->noeud == NULL)){
        //Determination direction dans laquelle chercher le noeud
        ArbreQuat **afils = direction(x, y, *a);
        return rechercheCreeNoeudArbre(R, afils, *a, x, y);
    }
}
...
Reseau* reconstitueReseauArbre(Chaines* C){
    ...
    //Liberation ArbreQuat (utilisé que ici)
    liberer_ArbreQuat(a);
